...
export default class Artists extends Component {
  constructor() {
    ...
    this.baseURL =
      "https://my-json-server.typicode.com/sondossamii/music-db/artists";
  ...
  renderArtists({ artists }) {
        ...
        return (
          <div className="col-12 col-md-6 col-lg-4 p-2" key={artist.id}>
            <div className="box">
              <img
                className="img-fluid"
                src={`/music-db/images/covers/${artist.cover}.jpg`}
                alt={`${artist.name} Cover`}
              />
              <button className="btn btn-primary rounded-pill box-btn">
                <Link to={`artist/${artist.id}`} style={{ color: "#fff" }}>
                  {artist.name}
                </Link>
              </button>
            </div>
    ...
    return <h2>There's a problem with Database</h2>;
  }
